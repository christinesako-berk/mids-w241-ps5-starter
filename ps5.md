...
1.  Run a crosstab of total\_ad\_exposures\_week1 and
    treatment\_ad\_exposures\_week1 to sanity check that the
    distribution of impressions looks as it should. Does it seem
    reasonable? Why does it look like this? (No computation required
    here, just a brief verbal response.)
...
4.  Implement the procedure you propose from part (c), run the placebo
    ...
    before wasting time proceeding.)
5.  Now estimate the causal effect of each ad exposure on purchases
    ...
    passed the placebo test in part (d).
...
7.  Estimate the causal effect of each treatment ad exposure on
    ...
    purchases during weeks 1 through 10).
8.  Estimate the causal effect of each treatment ad exposure on
    ...
    only during week 2 through week 10, inclusive.
9.  Tell a story that could plausibly explain the result from part (h).
...
8.  Tell a concrete story about what could be leading to the result in
    part (g).
9.  Tell a concrete story about how this differential attrition might
    bias our estimates.

# 3\. Think about Treatment Effects
...
outcomes? Make the strongest case you can for why this is *good*
measure.
